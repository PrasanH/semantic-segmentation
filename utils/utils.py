...
def vis_grid_4x3(model, data_path: str) -> None:
    ...
    colors = torch.as_tensor(
        [white, green, brown, gray, forest_green, red, dark_green, sky]
    )
...
def run_inference(model, image):
    ...
    if isinstance(image, np.ndarray):
        return Image.fromarray(output_predictions.byte().cpu().numpy()).resize(
...
def create_custom_colormap() -> np.ndarray:
    """Creates a custom label colormap for segmentation visualization.
    ...
    # Create a colormap array and fill with custom colors
    colormap = np.zeros((256, 3), dtype=int)  # 256 possible labels with RGB
